...
import { CreatePackageRequest, Package, useCreatePackage, useDeletePackage, useEditPackage, usePackages } from "@/service/packages";
...
interface CreatePackageRequestForm extends Omit<CreatePackageRequest, "client" | "expiration_date"> {
...
const CreatePackageForm: FC<CreatePackageFormProps> = ({
  ...
  return (
    <form
      ...
      className="space-y-4"
    >
      <Input placeholder="Hardware ID" autoComplete="name"
        ...
        <Table>
          ...
          <TableBody>
            {packages.map((p) => (
              <TableRow key={p.id}>
                <TableCell>
                  <div className="font-medium">{p.hardware_id.length ? p.hardware_id : 'No hardware ID'}</div>
                  <div className="text-sm text-muted-foreground flex flex-wrap gap-2">
                    <Badge>
                      Active from {fmt(p.created_at)} to {fmt(p.expiration_date)}
                    </Badge>
                  </div>
                </TableCell>
